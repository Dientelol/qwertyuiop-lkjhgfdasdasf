...
exports.run = (client, message, args) => { //Collecting info about command
  ...
  if (args.length == 0) { // If no args
    ...
    message.channel.send({
      embed: {
        "description": "Enviei a lista de comandos para si por mensagem privada.",
        "color": "#ff0000"
      }
    ...
    if (groupFound != "") {
      ...
      message.channel.send({
        embed: {
          "description": "Enviei a lista comandos para si por mensagem privada.",
          "color": 0xff0000
        }
      }).then(msg => {
        if (conf[message.guild.id].delete == 'true') {
