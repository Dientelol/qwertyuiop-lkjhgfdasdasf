...
try {
  ...
  console.log("[ERRO] Cards overwrited");
  const cards = {}
  ...
  if (args.includes("-color")) {
    ...
    if (color.length == 7) {
      ...
      message.channel.send({
        embed: {
          "title": "Success",
          "description": "A cor do seu rank mudou com sucesso! \nNova cor - \`" + color + "\`",
          "color": 0x22ff22
        }
      });
    } else {
  ...
  if (args.includes("-image")) {
    if (args[args.indexOf("-image") + 1] == "" || args[args.indexOf("-image") + 1] == null) {
      message.channel.send({
        embed: {
          "title": "Sucesso",
          "description": "A imagem da sua carta de rank foi removida com sucesso.",
          "color": 0x22ff22
        }
      });
    ...
    message.channel.send({
      embed: {
        "title": "Sucesso",
        "description": "A imagem da sua carta de rank foi modificada com sucesso.!\n*Use imagens de \`934x282\` images for best results*",
        "color": 0x22ff22
      }
    });}
  }
    ...
    ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
      ...
      jimp.read(urlBG, (err, imageBG) => {
        ...
        imageBG.getBuffer(jimp.MIME_PNG, (err, bufBG) => {
          ...
          ctx.fillStyle = colorRank;
          ctx.fillRect(0, 0, 0, 282);

          ctx.globalAlpha = opacity;
          ctx.fillStyle = "#2f3136";
          ctx.fillRect(0, 0, 934, 282);
          ctx.fillStyle = "#36393f";
          ctx.fillRect(0, 36, 934, 210);
          ctx.globalAlpha = 1;
