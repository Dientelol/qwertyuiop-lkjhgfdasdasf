...
exports.run = async (client, message, args, ops) => { //Collecting info about command
  ...
  if (config[message.guild.id].djonly == true) {
    if (!message.member.roles.some(r=>config[message.guild.id].djroles.includes(r.id))) return message.channel.send({ 
      embed: {
        "title": "Erro", 
        "description": "Neste servidor, o modo \"ApenasDJ\" está ativado.\nVocê não tem o cargo, por isso não pode escolhar músicas.\n*Para conhecer a lista de cargos DJ, use `" + config[message.guild.id].prefix + "dj`*", 
      } 
    });
  }
  ...
  if (!message.member.voiceChannel) {
    return message.channel.send({
      embed: {
        "title": "Erro",
        "description": "Você tem de estar num canal de voz para executar este comando.",
        "color": 0xff2222
      }
    }).then(msg => {
  ...
  if (!song) {
    return message.channel.send({
      embed: {
        "title": "Erro",
        "description": "Insira um nome de música ou um link.",
        "color": 0xff2222
      }
    }).then(msg => {
      if (config[message.guild.id].delete == 'true') {
  ...
  if (!data.dispatcher) {
    ...
    message.channel.send(new Discord.RichEmbed()
      ...
      .setDescription("Adicionado á fila: **" + info.title + "**")).then(msg => {
      if (config[message.guild.id].delete == 'true') {
        msg.delete(config[message.guild.id].deleteTime);
      }
    });
...
async function play(client, ops, data, streamOptions) {
  ...
  client.channels.get(data.queue[0].announceChannel).send(new Discord.RichEmbed()
    ...
    .setDescription("Está a dar: **" + data.queue[0].songTitle + "**")).then(msg => {
